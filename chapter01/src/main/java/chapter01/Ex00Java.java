...
public class Ex00Java {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 개발 시 JDK download(JDK에 JRE 포함됨)
		 * python은 별도의 compiler나 link(중간파일) 필요없이 interpreter만 있으면 아무 OS에서  실행 가능
		 * C는 별도의 compiler, link 필요하여 실행시키려는 OS에 맞는 compiler 필요
		 * java는 'javac'이라는 compiler가 필요하며 OS 상관없이 jvm만 있으면 linux javac이어도 window에서 실행 가능
		 * 중간에 java가 바이트 파일을 만들어줌. -> 플랫폼 독립적(유연성이 좋다)
		 * 
		 */
